...
describe("Ressource Materiel routes", () => {
  describe("DELETE /v1/materiel/ressource/:id", () => {
    // Initiation de la création d'une ressource d'un matériel
    let supprimerRessourceMateriel;

    beforeEach(async () => {
      supprimerRessourceMateriel = {
        id_ressource: "",
        id_materiel: "",
      };
    });
    ...
    test("devrait retourner 200 et supprimer avec succès une ressource_materiel si les données sont correctes.", async () => {
      const res = await request(app)
        .delete("/v1/materiel/resssource/:id")
        .send(supprimerRessourceMateriel);

      // vérifie si ça renvoie bien une 200
      expect(res.status).toEqual(200);

      // vérifie que la réponse du res.body.message correspond bien avec la ressource d'un materiel qui a été crée dans la supprimerRessourceMateriel
      expect(res.body.message).toEqual({
        id: expect.anything(),
        id_ressource: supprimerRessourceMateriel.id_ressource,
        id_materiel: supprimerRessourceMateriel.id_materiel,
      });
    });
    ...
    test("devrait retourner 404 => url non trouvé", async () => {
      const res = await request(app).delete("/v1/materiel/resssourceee").send({
        id_ressource: "",
        id_materiel: "",
      });
