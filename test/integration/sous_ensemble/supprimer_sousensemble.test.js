...
describe("Sous Ensemble routes", () => {
  describe("DELETE /v1/sousensemble/:id", () => {
    // Initiation de la création d'un sous_ensemble
    let supprimerSousEnsemble;

    beforeEach(async () => {
      supprimerSousEnsemble = {
        nom: "Sous_ensemble 8",
        id_installation: "",
      };
    });
    ...
    test("devrait retourner 200 et créer avec succès unn nouveau sous_ensemble si les données sont correctes.", async () => {
      const res = await request(app)
        .delete("/v1/sousensemble/:id")
        .send(supprimerSousEnsemble);

      // vérifie si ça renvoie bien une 200
      expect(res.status).toEqual(200);

      // vérifie que la réponse du res.body.message correspond bien avec le sous_ensemble qui a été crée dans le supprimerSousEnsemble
      expect(res.body.message).toEqual({
        id: expect.anything(),
        nom: supprimerSousEnsemble.nom,
        id_installation: supprimerSousEnsemble.id_installation,
      });
    });

    // test erreur 400 => un champs est manquant
    test("devrait retourner 400 => element manquant / joi validation error", async () => {
      const res = await request(app).delete("/v1/sousensemble/:id").send({
        nom: "Installation 8",
        id_installation: "",
      });
    ...
    test("devrait retourner 404 => url non trouvé", async () => {
      const res = await request(app).delete("/v1/sousensembleee").send({
        nom: "Installation 8",
        id_installation: "",
      });
