...
describe("Sous Ensemble routes", () => {
  describe("POST /v1/sousensemble", () => {
    // Initiation de la création d'un sous_ensemble
    let nouveauSousEnsemble;
    ...
    // test succès 200 => création d'un sous_ensemble
    test("devrait retourner 200 et créer avec succès unn nouveau sous_ensemble si les données sont correctes.", async () => {
      const res = await request(app)
        .post("/v1/sousensemble")
        .send(nouveauSousEnsemble);

      // vérifie si ça renvoie bien une 200
      expect(res.status).toEqual(200);

      // vérifie que la réponse du res.body.message correspond bien avec le sous_ensemble qui a été crée dans le nouveauSousEnsemble
      expect(res.body.message).toEqual({
        id: expect.anything(),
        nom: nouveauSousEnsemble.nom,
        id_installation: nouveauSousEnsemble.id_installation,
      });
    });

    // test erreur 400 => un champs est manquant
    test("devrait retourner 400 => element manquant / joi validation error", async () => {
      const res = await request(app).post("/v1/sousensemble").send({
        nom: "Installation 8",
        id_installation: "",
      });
    ...
    test("devrait retourner 404 => url non trouvé", async () => {
      const res = await request(app).post("/v1/sousensembleee").send({
        nom: "Installation 8",
        id_installation: "",
      });
